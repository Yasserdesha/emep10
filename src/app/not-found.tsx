...
export default function NotFound() {
  ...
  return (
    <div 
      ...
      dir={isMounted ? (isAr ? 'rtl' : 'ltr') : 'rtl'}
    >
      {/* Background ambient lighting */}
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[650px] h-[650px] bg-[#FF1E27]/12 blur-[150px] rounded-full" />
      </div>
      ...
      {/* 404 card */}
      <main className="max-w-[560px] w-full p-6 sm:p-10 rounded-3xl bg-[#111116]/95 border border-white/[0.08] shadow-[0_20px_60px_rgba(0,0,0,0.85)] relative overflow-hidden z-10 backdrop-blur-xl space-y-6 my-auto">
        {/* Glow Top Line */}
        <div className="absolute top-0 inset-x-0 h-1 bg-gradient-to-r from-[#D31019] via-[#FF1E27] to-[#FF4040]" />

        {/* Warning Icon Wrapped with ElectricBorder */}
        <div className="flex justify-center">
          <ElectricBorder
            ...
            className="max-w-fit"
          >
            <div className="w-14 h-14 bg-[#FF1E27]/15 border border-[#FF1E27]/30 rounded-[18px] flex items-center justify-center text-[#FF1E27] shadow-[0_0_35px_rgba(211,16,25,0.35)]">
        ...
        {/* 404 Big Number with Interactive FuzzyText Glitch Effect */}
        <div className="py-2 flex justify-center items-center overflow-visible">
          <FuzzyText
            fontSize="clamp(4.5rem, 14vw, 7.5rem)"
            fontWeight={900}
            gradient={['#FFFFFF', '#FF3B42', '#D31019']}
            baseIntensity={0.18}
            hoverIntensity={0.65}
            enableHover={true}
            clickEffect={true}
        ...
        <div className="space-y-2">
          ...
          <p className="text-xs sm:text-sm text-[#94A3B8] leading-relaxed max-w-sm mx-auto">
            {isAr
              ? 'عذراً، الرابط الذي تحاول الوصول إليه غير متاح أو تم نقله. يمكنك العودة للصفحة الرئيسية أو التواصل معنا.'
              : 'The engineering page or resource you are looking for does not exist or has been moved.'}
          </p>
        </div>
        ...
        <div className="flex gap-3 flex-col sm:flex-row justify-center pt-2">
          <SpecularButton
            ...
            autoAnimate
            onClick={() => router.push('/')}
            className="flex-1 py-3 text-xs font-bold shadow-lg shadow-[#FF1E27]/25"
          >
          ...
          <SpecularButton
            ...
            className="flex-1 py-3 text-xs font-bold bg-white/[0.04] text-white"
          >
